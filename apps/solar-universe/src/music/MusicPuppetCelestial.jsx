...
function TaperedCylinder({ start, end, radiusStart, radiusEnd, segments, color, roughness = 0.75 }) {
  ...
  return (
    <group position={[x, 0.12, 0.54]}>
      <mesh scale={[0.165, 0.142, 0.058]}>
        <sphereGeometry args={[1, segments, Math.max(10, Math.floor(segments * 0.65))]} />
  ...
  return (
    <group position={[0, 0.08, 0]} rotation={[-0.12, 0.29, -0.24]}>
      ...
      <mesh position={[0, -0.43, 0.3]} scale={[0.34, 0.24, 0.25]}>
        <sphereGeometry args={[1, quality === 'quality' ? 32 : 18, quality === 'quality' ? 20 : 12]} />
      ...
      {[-1, 1].map((side) => (
        <group key={`ear-${side}`} position={[side * 0.57, -0.02, 0.13]} rotation={[0, side * -0.18, side * 0.06]}>
          <mesh scale={[0.09, 0.15, 0.06]}>
            <sphereGeometry args={[1, quality === 'quality' ? 24 : 14, quality === 'quality' ? 18 : 10]} />
      ...
      <mesh position={NOSE_POINTS[3]} scale={[0.05, 0.043, 0.057]}>
        <sphereGeometry args={[1, quality === 'quality' ? 24 : 14, quality === 'quality' ? 16 : 10]} />
      ...
      <mesh position={[-0.31, 0.48, 0.26]} rotation={[0.08, 0.08, 0.38]} scale={[0.23, 0.34, 0.18]}>
        <sphereGeometry args={[1, quality === 'quality' ? 28 : 16, quality === 'quality' ? 18 : 10]} />
      ...
      <pointLight
        position={[1.25, 1.05, 2.2]}
        color="#f1b17a"
        intensity={selected ? 0.52 : 0.38}
        distance={5}
        decay={2}
      />
      <pointLight
        position={[-1.15, 0.32, 1.05]}
        color="#7b95a2"
        intensity={selected ? 0.16 : 0.1}
        distance={4}
        decay={2}
      />
    </group>

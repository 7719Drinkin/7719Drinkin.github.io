...
const worldThemes = {
  basketball: {
    kind: 'basketball-world', worldName: 'THE LAST COURT', css: '#d7a064', size: .98,
    orbitColor: 0x78513a, orbitRadius: 7.4, orbitalInclination: -.08,
    initialOrbitAngle: .64, orbitalAngularVelocity: .046,
    axialTilt: THREE.MathUtils.degToRad(-9), initialAxialAngle: .2, axialAngularVelocity: .018
  },
  games: {
    kind: 'games', worldName: 'STRATEGY WORLD', css: '#79ceff', size: .72,
    orbitColor: 0x2f779d, orbitRadius: 12.3, orbitalInclination: .035,
    initialOrbitAngle: 2.62, orbitalAngularVelocity: .036,
    axialTilt: THREE.MathUtils.degToRad(12), initialAxialAngle: 1.1, axialAngularVelocity: .015
  },
  music: {
    kind: 'music', worldName: 'SOUND FOREST', css: '#cf9fff', size: .77,
    orbitColor: 0x7a4b9d, orbitRadius: 17.1, orbitalInclination: .09,
    initialOrbitAngle: 4.72, orbitalAngularVelocity: .029,
    axialTilt: THREE.MathUtils.degToRad(-17), initialAxialAngle: 2.3, axialAngularVelocity: .013
  },
  default: {
    kind: 'default', worldName: 'UNMAPPED WORLD', css: '#c5ddec', size: .7,
    orbitColor: 0x647a8a, orbitRadius: 21, orbitalInclination: 0,
    initialOrbitAngle: 1.3, orbitalAngularVelocity: .024,
    axialTilt: THREE.MathUtils.degToRad(7), initialAxialAngle: 0, axialAngularVelocity: .012
  }
};
...
function createSun() {
  ...
  scene.add(sunMesh);
  scene.add(new THREE.PointLight(0xffb06a, ecoMode ? 92 : 128, 115, 1.7));
  const glow = new THREE.Sprite(new THREE.SpriteMaterial({
    map: createGlowTexture(),
    transparent: true,
    opacity: ecoMode ? .54 : .68,
    depthWrite: false,
...
function createLocalTube(points, radius, material, segments = 56) {
  const curve = new THREE.CatmullRomCurve3(
    points.map(([x, y]) => new THREE.Vector3(x, y, .018)),
    false,
    'centripetal'
  );
...
function createNumberCanyon(radius) {
  ...
  const basin = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x211711, roughness: 1, metalness: 0, transparent: true, opacity: .96, side: THREE.DoubleSide })
  );
  basin.scale.set(1.34, .9, 1);
  basin.position.z = .006;
  anchor.add(basin);

  const wallMaterial = new THREE.MeshStandardMaterial({ color: 0x8b654a, roughness: .98, metalness: 0 });
  const trenchMaterial = new THREE.MeshStandardMaterial({ color: 0x090604, roughness: 1, metalness: 0 });
  const glowMaterial = new THREE.MeshBasicMaterial({ color: 0xf09a55, transparent: true, opacity: .62, blending: THREE.AdditiveBlending, depthWrite: false });
  ...
  const walls = [createLocalTube(two, .064, wallMaterial), createLocalTube(three, .064, wallMaterial)];
  const trenches = [createLocalTube(two, .046, trenchMaterial), createLocalTube(three, .046, trenchMaterial)];
  const glows = [createLocalTube(two, .01, glowMaterial), createLocalTube(three, .01, glowMaterial)];
  walls.forEach((mesh) => { mesh.position.z = .014; anchor.add(mesh); });
  trenches.forEach((mesh) => { mesh.position.z = .022; anchor.add(mesh); });
  glows.forEach((mesh) => { mesh.position.z = .029; anchor.add(mesh); });

  const rockMaterial = new THREE.MeshStandardMaterial({ color: 0x5f4637, roughness: 1, flatShading: true });
  const random = seededRandom(2306);
  const rockCount = ecoMode ? 22 : 44;
...
function createChampionshipSanctuary(radius) {
  ...
  const stoneMaterial = new THREE.MeshStandardMaterial({ color: 0x594538, roughness: .94, flatShading: true });
  const darkMaterial = new THREE.MeshStandardMaterial({ color: 0x2a211b, roughness: .88, metalness: .12 });
  const goldMaterial = new THREE.MeshStandardMaterial({ color: 0xd0a45b, emissive: 0x3b2108, emissiveIntensity: .38, roughness: .31, metalness: .58 });
  ...
  const trophyBalls = [];
  const positions = [[-.2,.02],[0,.02],[.2,.02],[-.2,-.13],[0,-.13],[.2,-.13]];
  ...
  anchor.add(arch);
  const sanctuaryLight = new THREE.PointLight(0xffbd6c, ecoMode ? .5 : .86, 2.8, 2);
...
function createCourtWorld(radius) {
  ...
  const planet = new THREE.Mesh(
    createRockySphere(radius, ecoMode ? 3 : quality.terrainDetail, [0x211c19, 0x45352c, 0x725744, 0x9b7658], 23),
    new THREE.MeshStandardMaterial({ vertexColors: true, flatShading: true, roughness: .95 })
  );
  root.add(planet);

  const terraceLow = new THREE.Mesh(new THREE.CylinderGeometry(.72, .86, .14, 14), new THREE.MeshStandardMaterial({ color: 0x60483a, roughness: .94, flatShading: true }));
  terraceLow.position.y = radius * .89;
  landmarks.add(terraceLow);
  const terraceHigh = new THREE.Mesh(new THREE.CylinderGeometry(.64, .73, .11, 14), new THREE.MeshStandardMaterial({ color: 0x805d49, roughness: .9, flatShading: true }));
  terraceHigh.position.y = radius * .975;
  landmarks.add(terraceHigh);
  const retaining = new THREE.Mesh(new THREE.BoxGeometry(1.13, .12, .7), new THREE.MeshStandardMaterial({ color: 0x3d3029, roughness: .94, flatShading: true }));
  retaining.position.y = radius;
  landmarks.add(retaining);
  const court = new THREE.Mesh(new THREE.BoxGeometry(1.02, .03, .59), new THREE.MeshStandardMaterial({ color: 0x764535, roughness: .8 }));
  court.position.y = radius * 1.055;
  landmarks.add(court);

  const lineMaterial = new THREE.MeshBasicMaterial({ color: 0xf3eadb, transparent: true, opacity: .94 });
  ...
  const standMaterial = new THREE.MeshStandardMaterial({ color: 0x3b2e28, roughness: .92 });
  for (const side of [-1, 1]) {
  ...
  const tunnelPortal = new THREE.Mesh(new THREE.TorusGeometry(.11, .025, 7, 18, Math.PI), new THREE.MeshStandardMaterial({ color: 0x302b27, roughness: .9 }));
  ...
  const tunnelLamp = new THREE.PointLight(0xffa052, ecoMode ? .35 : .62, 1.35, 2);
  tunnelLamp.position.set(0, .08, .06);
  tunnelAnchor.add(tunnelLamp);

  const spectatorMaterial = new THREE.MeshBasicMaterial({ color: 0xffc17a, transparent: true, opacity: .62 });
  ...
  const shotTrail = new THREE.Mesh(new THREE.TubeGeometry(shotCurve, ecoMode ? 24 : 56, .007, 5, false), new THREE.MeshBasicMaterial({ color: 0xffb66d, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false }));
  const shotBall = new THREE.Mesh(new THREE.SphereGeometry(.022, 10, 8), new THREE.MeshBasicMaterial({ color: 0xffdc9b, transparent: true, opacity: 0, blending: THREE.AdditiveBlending }));
...
function createGamesWorld(radius) {
  const root = new THREE.Group();
  const geometry = createRockySphere(radius, ecoMode ? 2 : 4, [0x07131d, 0x16354a, 0x24566d, 0x2f6f87], 77);
...
function createMusicWorld(radius) {
  const root = new THREE.Group();
  const geometry = createRockySphere(radius, ecoMode ? 2 : 4, [0x120b1c, 0x2c1840, 0x25435a, 0x41647a], 91);
...
function createGenericWorld(radius) {
  const root = new THREE.Group();
  const geometry = createRockySphere(radius, ecoMode ? 2 : 3, [0x28343d, 0x465864, 0x647987, 0x7d919e], 123);
...
function createWorld(interests, interest, index) {
  const fallbackTheme = {
    ...worldThemes.default,
    orbitRadius: worldThemes.default.orbitRadius + index * 4.5,
    initialOrbitAngle: 1.3 + index * 1.8,
    initialAxialAngle: index * .7
  };
  const theme = worldThemes[interest.theme] ?? fallbackTheme;
  const orbit = createOrbit({ ...theme, inclination: theme.orbitalInclination });
  scene.add(orbit);

  const orbitalPlane = new THREE.Group();
  orbitalPlane.rotation.z = theme.orbitalInclination;
  scene.add(orbitalPlane);
  const orbitalPivot = new THREE.Group();
  orbitalPivot.rotation.y = theme.initialOrbitAngle;
  orbitalPlane.add(orbitalPivot);

  const orbitalCarrier = new THREE.Group();
  orbitalCarrier.position.x = theme.orbitRadius;
  orbitalPivot.add(orbitalCarrier);
  const axialTilt = new THREE.Group();
  axialTilt.rotation.z = theme.axialTilt;
  orbitalCarrier.add(axialTilt);

  const model = createWorldModel(theme);
  model.root.rotation.y = theme.initialAxialAngle;
  axialTilt.add(model.root);
  ...
  const entry = {
    ...
    label,
    index,
    initialOrbitAngle: theme.initialOrbitAngle,
    orbitalAngularVelocity: theme.orbitalAngularVelocity,
    initialAxialAngle: theme.initialAxialAngle,
    axialAngularVelocity: theme.axialAngularVelocity
  };
...
function buildScene(interests) {
  ...
  scene.add(camera);
  const cameraFill = new THREE.DirectionalLight(0xdce8ff, ecoMode ? .48 : .68);
  ...
  renderer.toneMappingExposure = ecoMode ? 1.18 : 1.3;
  ...
  scene.add(new THREE.HemisphereLight(0xa9c2e8, 0x4a2d20, ecoMode ? .8 : 1.05));
  scene.add(new THREE.AmbientLight(0x60718c, ecoMode ? .11 : .17));
  createSun();
...
function updateBasketballWorld(entry, elapsed, delta) {
  ...
  model.spectators.material.opacity = .54 + Math.sin(elapsed * 1.25) * .09;
  model.floodHeads.forEach((head, index) => {
    const wave = .5 + .5 * Math.sin(elapsed * 1.15 - index * .78);
    head.material.emissiveIntensity = .9 + wave * 1.35;
  });
  const canyonPulse = .42 + (.5 + .5 * Math.sin(elapsed * .55)) * .22;
  model.canyonGlow.forEach((stroke) => { stroke.material.opacity = canyonPulse; });
  model.trophyBalls.forEach((ball, index) => {
    const pulse = 1 + Math.sin(elapsed * .7 + index * .65) * .035;
...
function animate(time) {
  ...
  if (!reduceMotion) {
    for (const entry of worlds) {
      entry.orbitalPivot.rotation.y = entry.initialOrbitAngle + elapsed * entry.orbitalAngularVelocity;
      entry.axialBody.rotation.y = entry.initialAxialAngle + elapsed * entry.axialAngularVelocity;

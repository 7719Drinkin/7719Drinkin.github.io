...
function createOrbit(theme) {
  ...
  const material = new THREE.LineBasicMaterial({ color: theme.orbitColor, transparent: true, opacity: ecoMode ? .045 : .06, depthWrite: false });
...
function createCanyonStroke(radius, centerNormal, points2d, color, width) {
  const n = centerNormal.clone().normalize();
  ...
  const points = points2d.map(([u, v]) => n.clone().addScaledVector(tangent, u).addScaledVector(bitangent, v).normalize().multiplyScalar(radius * 1.006));
  const curve = new THREE.CatmullRomCurve3(points, false, 'centripetal');
  return new THREE.Mesh(new THREE.TubeGeometry(curve, ecoMode ? 24 : 56, width, 5, false), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: .72 }));
}
...
function createCourtWorld(radius) {
  ...
  const rockGeometry = createRockySphere(radius, ecoMode ? 3 : quality.terrainDetail, [0x171412, 0x352820, 0x5b4434, 0x806147], 23);
  const planet = new THREE.Mesh(rockGeometry, new THREE.MeshStandardMaterial({ vertexColors: true, flatShading: true, roughness: .96, metalness: 0 }));
  root.add(planet);
  ...
  retaining.position.y = radius * 1.0;
  landmarks.add(retaining);
  const court = new THREE.Mesh(new THREE.BoxGeometry(1.02, .03, .59), new THREE.MeshStandardMaterial({ color: 0x5c3328, roughness: .82, metalness: .01 }));
  court.position.y = radius * 1.055;
  landmarks.add(court);

  const lineMaterial = new THREE.MeshBasicMaterial({ color: 0xe5dccc, transparent: true, opacity: .88 });
  const lineHeight = .006;
  for (const z of [-.278, .278]) {
    const line = createCourtLine(.96, .011, lineHeight, lineMaterial);
    line.position.set(0, radius * 1.073, z);
    landmarks.add(line);
  }
  for (const x of [-.474, 0, .474]) {
    const line = createCourtLine(.011, .55, lineHeight, lineMaterial);
  ...
  for (const side of [-1, 1]) {
    for (let tier = 0; tier < 3; tier += 1) {
      const arcRadius = .36 + tier * .06;
      for (let segment = -3; segment <= 3; segment += 1) {
        const stand = createArcStand(arcRadius, segment * .18, radius * 1.055 + tier * .035, side * (.36 + tier * .045), standMaterial);
        landmarks.add(stand);
      }
    }
  }
  ...
  const stairsMaterial = new THREE.MeshStandardMaterial({ color: 0x6d5541, roughness: .92 });
  for (let index = 0; index < 7; index += 1) {
    const stair = new THREE.Mesh(new THREE.BoxGeometry(.16, .025, .09), stairsMaterial);
    stair.position.set(.66, radius * .91 + index * .025, -.12 + index * .02);
    stair.rotation.y = -.35;
    landmarks.add(stair);
  }

  const shrineAnchor = createSurfaceAnchor(radius, new THREE.Vector3(-.72, .24, -.65), .02);
  root.add(shrineAnchor);
  const shrineBase = new THREE.Mesh(new THREE.CylinderGeometry(.16, .2, .06, 8), new THREE.MeshStandardMaterial({ color: 0x3e3026, roughness: .9 }));
  shrineBase.position.y = .03;
  shrineAnchor.add(shrineBase);
  const goldMaterial = new THREE.MeshStandardMaterial({ color: 0x9d773d, roughness: .5, metalness: .46, emissive: 0x221204, emissiveIntensity: .2 });
  const crown = new THREE.Group();
  crown.position.y = .07;
  shrineAnchor.add(crown);
  for (let index = 0; index < 6; index += 1) {
    const angle = index / 6 * Math.PI * 2;
    const fin = new THREE.Mesh(new THREE.ConeGeometry(.035, .28 + index * .01, 5), goldMaterial);
    fin.position.set(Math.cos(angle) * .105, .14, Math.sin(angle) * .105);
    fin.rotation.z = Math.cos(angle) * .28;
    fin.rotation.x = Math.sin(angle) * .28;
    crown.add(fin);
  }
  const shrineLight = new THREE.PointLight(0xffa94d, ecoMode ? .32 : .58, 1.8, 2);
  shrineLight.position.y = .22;
  shrineAnchor.add(shrineLight);
  ...
  const canyonNormal = new THREE.Vector3(-.12, -.22, .97);
  const two = [[-.18,.14],[-.08,.22],[.08,.18],[.14,.08],[.06,-.02],[-.12,-.19],[.16,-.19]];
  const three = [[.23,.19],[.39,.22],[.49,.12],[.39,.02],[.48,-.08],[.39,-.2],[.22,-.17]];
  const canyon2 = createCanyonStroke(radius, canyonNormal, two, 0x110c09, .018);
  const canyon3 = createCanyonStroke(radius, canyonNormal, three, 0x110c09, .018);
  root.add(canyon2, canyon3);
  const canyonGlow2 = createCanyonStroke(radius * 1.002, canyonNormal, two, 0xb56b32, .004);
  const canyonGlow3 = createCanyonStroke(radius * 1.002, canyonNormal, three, 0xb56b32, .004);
  canyonGlow2.material.opacity = .28;
  canyonGlow3.material.opacity = .28;
  root.add(canyonGlow2, canyonGlow3);

  const spectatorMaterial = new THREE.MeshBasicMaterial({ color: 0xffb263, transparent: true, opacity: .54 });
  const spectatorGeometry = new THREE.SphereGeometry(.009, 5, 4);
  const spectators = new THREE.InstancedMesh(spectatorGeometry, spectatorMaterial, ecoMode ? 22 : 58);
  ...
  landmarks.add(shotBall);

  return { root, planet, landmarkGroup: landmarks, crown, spectators, floodHeads, canyonGlow: [canyonGlow2, canyonGlow3], shot: { curve: shotCurve, trail: shotTrail, ball: shotBall } };
}
...
function createGamesWorld(radius) {
  ...
  const planet = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ vertexColors: true, flatShading: true, roughness: .72, metalness: .18 }));
  root.add(planet);
...
function createMusicWorld(radius) {
  ...
  const planet = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ vertexColors: true, flatShading: true, roughness: .65, metalness: .12 }));
  root.add(planet);
  const rings = [];
...
function updateBasketballWorld(entry, elapsed, delta) {
  ...
  model.crown.rotation.y += delta * .035;
  ...
  const canyonPulse = .18 + (.5 + .5 * Math.sin(elapsed * .55)) * .16;
...
function animate(time) {
  ...
  if (!reduceMotion) {
    for (const entry of worlds) {
      ...
      const selectedFactor = entry === selected ? .22 : 1;
      entry.root.rotation.y += delta * entry.spinSpeed * selectedFactor;
